...
from wtforms import SubmitField, StringField, EmailField, SelectField, SelectMultipleField, widgets
...
def feed():
    ...
    if request.method == 'POST':
        ...
        if mem and f'i {session} i' not in mem.likers:
    ...
    return render_template('feed.html', title='Лента новостей', fe=fe, adv=adv, avatar=get_avatar(), form=form,
                           cat=cats, sort=sor)
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        with open("data/info.json", 'w') as file:
            json.dump(data, file)
        fn = f'{n + 1}.{razr}'
        filename = photos.save(form.photo.data, name=fn)
